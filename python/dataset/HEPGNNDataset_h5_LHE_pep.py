...
class HEPGNNDataset_h5_LHE_pep(PyGDataset):
    def __init__(self, **kwargs):
        super(HEPGNNDataset_h5_LHE_pep, self).__init__(None, transform=None, pre_transform=None)
        self.isLoaded = False
...
#             print("Loading files... (%d/%d) %s" % (i+1,nFiles,fName), end='\r')
            ...
            for j in range(nEvent):
                pt_fea = np.sqrt(f_px[j]**2+f_py[j]**2)
                eta_fea = np.arcsinh(np.nan_to_num(f_pz[j]/np.sqrt(f_px[j]**2+f_py[j]**2)))
                phi_fea = np.arctan2(f_py[j],f_px[j])
                
                f_fea_reshape = torch.cat((torch.from_numpy(f_m[j]).reshape(-1,1),torch.from_numpy(pt_fea).reshape(-1,1),torch.from_numpy(eta_fea).reshape(-1,1), torch.from_numpy(phi_fea).reshape(-1,1)),1).float()    
