...
class HEPGNNDataset_h5_LHE_draw(PyGDataset):
    def __init__(self, **kwargs):
        super(HEPGNNDataset_h5_LHE_draw, self).__init__(None, transform=None, pre_transform=None)
        self.isLoaded = False
...
#                 f_fea_reshape = torch.cat((torch.from_numpy(f_spin[j]).reshape(-1,1),torch.from_numpy(f_id[j]).reshape(-1,1)),1).float()   
