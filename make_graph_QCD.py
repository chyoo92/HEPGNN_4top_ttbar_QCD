...
## make data    
    for j in range(len(FPt)):
       ...
       # x_fea = torch.Tensor([np.array(JetPt[j]), np.array(JetEta[j]), np.array(JetPhi[j]), np.array(JetMass[j]), np.array(JetBTag[j]), NBjets,np.ones((len(JetPt[j]),))]).transpose(0,1)
